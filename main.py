    ...
    for symbol in SYMBOLS:
        ...
        try:
            ...
            for i in range(1, len(df_slice)):
                ...
                ema200 = df_slice['ema200'].iloc[i]
                mom = df_slice['momentum'].iloc[i]

                sqz_start = df_slice['sqzStart'].iloc[i]
                sqz_fired = df_slice['sqzFired'].iloc[i]
                ...
                if sqz_start:
                    if mom > 0 and (not USE_TREND or close_p > ema200):
                        ...
                        if sig_id not in sent_signals:
                            msg = (f"⏳ *WAIT CE (Squeeze Consolidating)* | {display_name}\n"
                                   f"Time: {candle_time}\n"
                        ...
                        if sig_id not in sent_signals:
                            msg = (f"⏳ *WAIT PE (Squeeze Consolidating)* | {display_name}\n"
                                   f"Time: {candle_time}\n"
                                   f"Current Price: ₹{close_p:.2f}\n"
